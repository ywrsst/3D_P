...
void Init()
{
	...
	for (float theta = 0; theta < 360; theta += 72)
	{
		...
		star[i].pos.vec[0][0] = -sin(radians(theta)) * 0.5f;      // x,y,z를 각 각 vec[0][0] vec[0][1] vec[0][2]로 바꿔준다.
	...
	{
		circle[theta].pos.vec[0][0] = sin(radians((float)theta)) * 0.5; //생성한 radians 함수를 sin,cos안에 넣는다
		circle[theta].pos.vec[0][1] = cos(radians((float)theta)) * 0.5; // x,y,z를 각 각 vec[0][0] vec[0][1] vec[0][2]로 바꿔준다.
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 360; i++)
		{
			transformedCircle[i].pos = circle[i].pos * transform.scale * transform.rotation * transform.translate;  
		     ...
		     // 원래  circle.pos , translate, rotation scale 순인대 내 코드의 연산 순서로 바꿔주었다.
		...
		int a = 0;
		glColor4f(transformedStar[a].r, transformedStar[a].g, transformedStar[a].b, transformedStar[a].a);   // x를 vec[0][0]으로 y를 vec[0][1]로 바꿔줌
